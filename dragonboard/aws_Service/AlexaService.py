...
import checking_connection as Connection
import yaml
import requests
import coloredlogs
import globals
...
def alexa_speech_recognizer(audio_stream):
	...
	try:
		test = requests.get("https://raw.githubusercontent.com/leachim6/hello-world/master/t/plain-text.txt", timeout=2)
		resp = requests.post(url, headers=headers, data=data)
	except requests.exceptions.ReadTimeout:
		print "[AlexaService]: Exception on the speech recognizer - Timeout connection."
	except requests.exceptions.RequestException:
		print "[AlexaService]: Exception on the speech recognizer."
	finally:
		platform.indicate_processing(False)
		process_response(resp)
		#print "finally speech recognizer"
	#print "executed code outside finally clause!"
...
def alexa_getnextitem(navigationToken):
	...
	try:
		...
		print "[AlexaService]: Get next item exception - Timeout."
	except requests.exceptions.RequestException:
		print "[AlexaService]: Get next item exception."
	finally:
			process_response(response)
			#print "Get next item finally"
...
def alexa_playback_progress_report_request(requestType, playerActivity, stream_id):
	...
	try:
		...
		print "[AlexaService]: Internet connection problem."
...
def coffee_machine_is_down():
	player.play_speech(resources_path + 'coffee_machine_down.mp3')
	print "[AlexaService]: Playing Audio - Coffee Machine is down."

def coffee_machine_is_up():
	player.play_speech(resources_path + 'coffee_machine_up.mp3')
	print "[AlexaService]: Playing Audio - Coffee Machine is up."

def lamp_stand_is_down():
	player.play_speech(resources_path + 'lamp_stand_down.mp3')
	print "[AlexaService]: Playing Audio - Lamp Stand is down."

def lamp_stand_is_up():
	player.play_speech(resources_path + 'lamp_stand_up.mp3')
	print "[AlexaService]: Playing Audio - Lamp Stand is up."

def internet_is_down():
	if(globals.internetState != False):
		player.play_speech(resources_path + 'internet_down.mp3')
		print "[AlexaService]: Playing Audio - Internet is down."
		globals.internetState = False

def internet_is_up():
	if(globals.internetState != True):
		player.play_speech(resources_path + 'internet_up.mp3')
		print "[AlexaService]: Playing Audio - Internet is up."
		globals.internetState = True
...
def convertText(input_text):
	...
	print("[AlexaService][convertText]: Generating speech to: "+input_text)
	tts = gTTS(input_text, lang='en')
	tts.save('custom.mp3')
	print("[AlexaService][convertText]: Generated MP3 file!")
	print("[AlexaService][convertText]: Converting MP3 to WAV...")
	...
	print("[AlexaService][convertText]: Playing back query...")
	subprocess.call(['aplay', 'custom.wav'])
	print("[AlexaService][convertText]: Sending query to Alexa...")
...
def sendAlexaText(message):
	global isSendText
	if message == '1':
		audioOn = open(resources_path + 'turn_on.wav', 'r')
		#subprocess.call(['aplay', 'turn_on.wav'])
		isSendText = TRUE
		alexa_speech_recognizer(audioOn)

	elif message == '0':
		audioOff = open(resources_path + 'turn_off.wav', 'r')
		isSendText = TRUE
