...
class PocketsphinxTrigger(BaseTrigger):
	...
	def thread(self):
		while True:
			...
			if triggered:
				try:
					print "[pocketsphinxtrigger]: Testing connection on trigger"
					response = os.system("ping -c 1 216.58.222.110")
					if response == 0:
						self._trigger_callback(self)
					else: 
						print "[pocketsphinxtrigger]: Exception when triggering - response"
						AlexaService.internet_is_down()
						triggered = False
				except Exception, e:
					print "[pocketsphinxtrigger]: Exception when triggering - exception."
					AlexaService.internet_is_down()
					triggered = False
	def enable(self):
